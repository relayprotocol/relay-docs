...
- Regenerate with `node scripts/build-changelog.mjs`; `--check` fails when the committed page differs from a fresh build. Both in-repo sources make this almost deterministic — the only external input is a shallow clone of relay-client for the App line — so `--check` is now viable as a PR gate on non-fork branches. Consequence to expect either way: a PR that edits a source file gets a preview without that entry, which appears after merge at the next `:17`.
