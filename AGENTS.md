...
- **A sticky filter bar sits above the timeline.** `script.js` injects it from the tags actually present on the page, in `TAG_ORDER`, with an `All` chip that clears the filter. Both chip sets drive the same state, so clicking either updates both. It sticks below the navbar using a height measured at runtime — Mintlify exposes no variable for it — and publishes its own height as `--cl-bar-height`, which `style.css` adds to Mintlify's `--scroll-mt` in a `calc()` on the day's sticky date column — that column parks at the same offset the bar occupies and would otherwise sit behind it. Adding rather than replacing keeps their value and its unit; reading the variable in JS and re-emitting it in pixels silently mis-converts a `rem` value. It borrows `bg-background-light dark:bg-background-dark` from Mintlify's own sticky elements for its background. Injected rather than generated into the MDX, because filtering needs the script anyway: no script, no bar, rather than an inert control.
- **The filter reaches inside a day.** A date that survives it still hides the sections and entries belonging to other product lines, down to individual bullets inside a change-type group. Sections map to tags by heading (`API`, `App`); inside `### RelayKit` the tags come from the package attribution the generator appends (`SDK 7.0.0`), matched only when a version follows the name so prose mentioning "the SDK" is not read as one. Anything with no attribution to go on stays visible — the filter narrows, it never guesses. All of this hangs off Mintlify's `data-component-part` attributes (`update-tag`, `update-tag-list`, `update-content`), verified against a deployed build; if an upgrade renames them the filter silently stops working, so re-check it after a Mintlify bump.
...
- **The sync opens a pull request, it does not push to `main`.** `bot/changelog-sync` is rebuilt as `main` plus one commit on every run, so the diff is always exactly what is pending; an existing open PR is updated in place, and is left untouched when the content has not changed. When `main` catches up the branch is deleted, which closes the PR. **The page therefore only updates when someone merges** — the sync is hourly, publication is not.
- Regenerate with `node scripts/build-changelog.mjs`; `--check` fails when the committed page differs from a fresh build. It clones relay-kit (full depth, for CHANGELOG history) and relay-client (shallow), so it needs network and a token but no registry; a PR gate is viable on non-fork branches, and unlike an npm-backed build it will not start failing the moment relay-kit publishes. Consequence to expect either way: a PR that edits a source file gets a preview without that entry, which appears after merge at the next `:17`.
