...
You can also choose to sponsor specific Relay fees for your users using the `sponsoredFeeComponents` parameter. This allows integrators to selectively cover different fee types (such as gas fees or relayer fees) while still collecting app fees:

```tsx
"sponsoredFeeComponents": ["gas", "relayer"]
```
