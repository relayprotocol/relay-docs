...
You can't always tell in advance which case applies, so poll by `depositAddress` rather than by the quote-time `requestId`. An open address can return several requests, one per deposit. Use `depositAddress.depositTxHash` to match each one to the transfer that funded it.
