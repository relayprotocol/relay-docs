...
Relay supports instantly and cheaply bridging from a variety of EVM and non-EVM chains to any token.
Deposit addresses allow users to bridge without connecting a wallet. Users can send funds to a designated address
and the protocol will instantly deposit the correct tokens on the destination chain in the user's wallet. This is a great way for users to quickly
start transacting on a new chain!

## How It Works

Once generated, a single deposit address can be reused so long as the origin currency, origin chain, destination currency, and destination chain all remain the same. To enable using a deposit address for your bridge, you will need to set the `useDepositAddress` body parameter to `true` in the [quote request](https://docs.relay.link/references/api/get-quote-v2).
...
Failures via deposit addresses are extremely rare, but are still possible. If something goes wrong, the protocol will default to manually handling reversions. If you wish to automatically receive reversed funds on the origin chain, specify the `refundTo` address. This will mean reversions will automatically be issued on origin in case of any filling failures.
...
33,000 extra gas is required than regular native bridges, and about 70,000 more for ERC-20 bridges. For very small bridges, deposit addresses are not recommended since so much gas is required.

Once a deposit occurs, the quote will automatically be generated post-deposit to account for gas fluctuation and recompute all fees. This can change the amount received to be different than the quoted value but will always be at least the minimum rate. If the minimum rate cannot be achieved, the deposit amount will revert.

All Relay EVM deposit addresses begin and end with the character `d` (not case-sensitive). This helps reduce the indexing search space.
...
- When non-supported assets or NFTs are sent to the deposit address, Relay currently does not support recovering of those assets.

## Debugging

When looking up the status of a bridge or fetching the request data, please use the `requestId` and the [Get Execution Status API](/references/api/get-intents-status-v3).
...
- `refundTo` can be optionally set when `useDepositAddress` is being used.
  This is the address that reversed funds will be sent to in case of an issue. If not specified then manual reversion is required.
...
- **CEX-to-DeFi Bridging** -- When users are already performing a withdrawal from a centralized exchange and want to bridge to a different chain without an additional transaction.
