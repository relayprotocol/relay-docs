...
Once generated a single deposit address can be reused so long as the input is the same currency and on the same chain. To enable using a deposit address for your bridge, you will need to set the `useDepositAddress` body parameter to `true` in the [quote request](https://docs.relay.link/references/api/get-quote-v2).
