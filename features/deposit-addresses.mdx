...
When using fee sponsorship with deposit addresses, the `user` and `depositFeePayer` addresses **cannot be the same wallet address**. This restriction is particularly important for same-chain Solana transactions.

- For same-chain transactions (origin and destination on the same chain), do not pass the `depositFeePayer` parameter
- Ensure the `user` and `depositFeePayer` are different addresses when fee sponsorship is required
- Violating this constraint will result in transaction failures during quote execution
...
- The `user` and `depositFeePayer` cannot be the same wallet address when using fee sponsorship.
