...
- **CEX-to-DeFi Bridging**: When users are already transferring funds from a centralized exchange and want to bridge to a destination chain without additional wallet interactions
- **Onboarding Flows from Fiat Rails**: Enabling seamless bridging as part of fiat-to-crypto onboarding without requiring complex wallet setup
- **Wallet Connection Issues**: When wallet connection or signing is problematic, such as when wallet providers don't support the origin chain
