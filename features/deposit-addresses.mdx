...
Deposit addresses let users bridge or swap tokens by simply sending funds to an address — no wallet connection or signing required. The integrator requests a quote with `useDepositAddress: true`, receives a deposit address, and the user transfers funds there. Relay detects the deposit and fills on the destination chain.
...
3. **Detect + Sweep** — Relay detects the deposit on-chain and sweeps funds to the depository contract. The amount, currency, and chain are validated and the quote is regenerated if different from the original.
...
| `refundTo` | address | Address to send refunds if the transfer fails. Required for strict deposit addresses. See [Refund Behavior](#refund-behavior). |
...
| `amount` | string | The quoted amount in the origin token's smallest unit. User may send a different amount — Relay will regenerate the quote. |
...
A single deposit address can be reused so long as the origin currency, origin chain, destination currency, and destination chain all remain the same. A new deposit to the same address triggers a fresh quote and fill.

## Open vs Strict Deposit Addresses

Relay supports two deposit address modes that differ in how flexible they are when handling unexpected deposits.

### Open Deposit Addresses

Open deposit addresses accept **any solver currency on any supported chain**. They provide maximum flexibility — if a user sends a different supported token or deposits on a different chain (same VM), Relay automatically regenerates the quote and fills.

- `refundTo` is optional. Relay sets an internal fallback address for rare failure cases.
- Best for: general-purpose integrations where you want maximum tolerance for user error.

### Strict Deposit Addresses

Strict deposit addresses are tied to a specific order. They are designed to accept **only the exact currency specified in the original quote**.

- `refundTo` is **required**. Since strict addresses have tighter conditions, the chance of a deposit needing a refund is higher.
...
|---|---|---|
| Accepted currencies | Any solver currency on any supported chain | Only the exact currency in the original quote |
| Wrong token (solver currency) | Auto quote regeneration and fill | Auto quote regeneration and fill |
| Wrong token (non-solver) | Not recoverable | Not recoverable |
| Wrong chain (same VM) | Auto-reroute, new `requestId` created | Auto-reroute, new `requestId` created |
| Wrong chain (different VM) | Not possible — address formats differ | Not possible — address formats differ |
| Amount mismatch | Quote regenerated for actual amount | Quote regenerated for actual amount |
| `refundTo` | Optional (internal fallback) | Required |
| Address reuse | Yes, same route | Yes, same route |
...
When funds arrive at a deposit address, Relay evaluates what was sent versus what was originally quoted. The system handles mismatches automatically:
...
- **Less than quoted** — If the smaller amount still covers fees and the minimum fill, a new quote is generated and the fill proceeds. If not, the deposit is refunded to the `refundTo` address.

### Different Token (Solver Currency)

If the user sends a different token that is a [solver currency](#caveats), Relay automatically regenerates the quote using the actual currency deposited and fills the order.

### Different Chain (Same VM)

If the user deposits on the wrong chain but the address exists on that chain (same VM type, e.g., EVM to EVM), Relay auto-reroutes the deposit. A new `requestId` is created for the rerouted order.
...
<Warning>
When Relay regenerates a quote (different amount, different token, or different chain), a **new `requestId` is generated**. Always track transactions by deposit address, not by `requestId`. See [Tracking Transactions](#tracking-transactions).
</Warning>
...
1. **Correct currency, fill failed** (e.g., slippage, network issues) — The deposit is automatically refunded to the `refundTo` address, minus the cost of gas. No additional fees are taken.
2. **Wrong currency (non-solver token)** — Automatic refunds are not possible because Relay can't collect the unsupported token to cover gas costs. The user must visit a recovery UI to reclaim funds. If the integrator is willing to sponsor gas costs, auto-refunds can be arranged.
...
| Not set (open only) | Relay sets an internal fallback and handles recovery for rare failures |
...
- **Open deposit addresses**: Set `refundTo` to the user's address for automatic refunds, or omit it to let Relay handle edge cases internally.
...
When a quote is regenerated (different amount, token, or chain), a new `requestId` is generated. Use `includeChildRequests=true` to find all related requests, including regenerated ones:
...
If a deposit appears stuck, you can trigger on-demand re-detection using the [Deposit Address Reindex](/references/api/deposit-address-reindex) endpoint. This checks all solver currency balances on the specified chain and triggers sweeps for any detected deposits:
...
Only tokens listed as **solver currencies** for a given chain can be processed by deposit addresses. Relay treats certain tokens as equivalent within currency groups (e.g., ETH and WETH) — depositing any token in a group triggers the same fill behavior.
...
- Only exact input bridging is supported (`tradeType: "EXACT_INPUT"`).
