...
The figures below assume you are sponsoring fees as well (`"subsidizeFees": true`, as in the request example at the end of this section). See [Fixed rates without fee sponsorship](#fixed-rates-without-fee-sponsorship) for the other case:
...
#### Fixed Rates Without Fee Sponsorship

`fixedRate` and `subsidizeFees` are independent settings — you can pass a fixed rate with `"subsidizeFees": false`. A fixed rate always pins the **pre-fee** conversion; `subsidizeFees` decides who pays the Relay fee on top of it.
...
#### Tracking What You Sponsored or Earned

Fixed-rate quotes return a **`fixedRateFee`** object on `details`, alongside `isFixedRate`:
