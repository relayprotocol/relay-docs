...
<Warning>
Deposit addresses do not support `sponsoredFeeComponents`. `maxSubsidizationAmount` is supported by deposit addresses but if the amount exceeds the maximum the user pays the full amount.
</Warning>
