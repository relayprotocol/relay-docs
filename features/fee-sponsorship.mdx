...
<Warning>
[Deposit addresses](/features/deposit-addresses) do not support `sponsoredFeeComponents` or `subsidizationBps`. `maxSubsidizationAmount` is supported by deposit addresses but if the amount exceeds the maximum the user pays all fees and nothing is subsidized.
</Warning>
...
<Info>
`subsidizationBps` is currently supported only on swap execution flows. Bridge and [deposit address](/features/deposit-addresses) flows return `400` when the field is set — use `maxSubsidizationAmount` for those flows instead.
</Info>
...
- **Solana ATA rent** — The `rent` component is user-paid unless you select it explicitly in `sponsoredFeeComponents` or set `subsidizeRent`. See [Solana Fee Sponsorship](#solana-fee-sponsorship).
...
<Note>
**Swap costs and app fees are sponsored by default.** With `subsidizeFees: true` and no **`sponsoredFeeComponents`**, the sponsor covers `execution`, `swap`, `relay`, and `app`:

- **Swap costs** (`swap`) cover DEX fees, DEX swap impact, and solver rebalancing. To leave them with the user, omit `swap` from `sponsoredFeeComponents`.
- **App fees** (`app`) are drawn from your app balance and still accrue to the recipients named in `appFees`. Omit `app` from `sponsoredFeeComponents` to keep collecting app fees from the user while sponsoring the rest.
</Note>
