...
The Allocator uses **MPC chain signatures** for signing, meaning no single entity holds the private keys needed to authorize withdrawals. It is governed by the [Security Council](/references/protocol/components/security-council), which can suspend individual withdrawers or replace the Allocator in an emergency.
...
sequenceDiagram
    ...
    Allocator-->>Solver: 7. Return signed withdrawal proof
    Solver->>Depository: 8. Submit proof to target chain
...
- **[Security Council](/references/protocol/components/security-council)** — A multisig where any single member can suspend a specific withdrawer, and a supermajority can replace the Allocator entirely.
