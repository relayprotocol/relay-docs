...
The Allocator uses **MPC chain signatures** for signing, meaning no single entity holds the private keys needed to authorize withdrawals. It is governed by the [Security Council](/references/protocol/components/security-council), which can suspend withdrawals or replace the Allocator in an emergency.
...
The `RelayAllocatorSpender` is the sole holder of `APPROVED_WITHDRAWER_ROLE` on the Allocator. It requires a valid **EIP-712 signature from a registered Oracle** before forwarding any withdrawal request. This means:
...
- **Suspension halts all withdrawals** — Because the `RelayAllocatorSpender` is the only address with `APPROVED_WITHDRAWER_ROLE`, suspending it effectively halts all withdrawal proof generation across the protocol
...
- **[Security Council](/references/protocol/components/security-council)** — A multisig where any single member can suspend the `RelayAllocatorSpender`, halting all withdrawal proof generation. A supermajority can replace the Allocator entirely.
