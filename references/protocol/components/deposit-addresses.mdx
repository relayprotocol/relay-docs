...
Each deposit address is deterministically derived from the **orderId** using CREATE2 and [EIP-1167](https://eips.ethereum.org/EIPS/eip-1167) minimal proxy clones. Because the address depends only on the factory address, the implementation address, and the orderId salt, the address can be computed **before any contract is deployed** there.
...
Once funds arrive, the Relay backend calls `deployAndSweep` on the DepositFactory. This atomically:
...
If the proxy has already been deployed (e.g., from a previous deposit to the same orderId), the factory calls `sweep` directly on the existing proxy — no redeployment needed.

## Contracts

### DepositFactory

The factory is deployed once per supported chain. It has three key functions:

- **`computeDepositAddress(orderId)`** — Returns the deterministic address where a user should send funds for a given order. This is a view function that requires no gas.
...
A single deposit address can receive multiple token types. The `deployAndSweep` and `sweep` functions accept an array of token addresses, sweeping each one to the Depository in a single transaction. The zero address (`0x0000000000000000000000000000000000000000`) represents native ETH.

## Security

- **Deterministic and verifiable** — Anyone can independently compute a deposit address from the orderId and verify it matches what the API returned
- **Atomic operations** — `deployAndSweep` is atomic: if any part fails, the entire transaction reverts and funds remain safe at the deposit address
