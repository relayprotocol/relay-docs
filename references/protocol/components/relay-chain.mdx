...
Settlement activity on the Relay Chain includes deposit attestations, fill and refund settlement executions, balance transfers, and withdrawal-burn executions. Withdrawal proof generation itself happens on Aurora via the [Allocator](/references/protocol/components/allocator), not on the Relay Chain.
