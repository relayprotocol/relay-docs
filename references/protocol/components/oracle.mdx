...
4. **Agregate**  — an Oracle instance has awareness of `PEERS` which are other Oracle instances and will trigger attestations from them as the Relay Oracle contract requires multiple attestations from different Oracle before validating an execution.
6. **Return** — The signed attestation is returned to the caller, who submits it to the Oracle contract on the Relay Chain. The contract verifies the threshold is met and executes the actions on the Hub
