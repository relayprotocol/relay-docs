...
Relay Protocol settles cross-chain intents. Users express what they want (e.g., bridge 1 ETH from Optimism to Base), and solvers fill those orders using their own capital. The protocol then settles solvers — verifying fills and reimbursing them from the user's escrowed deposit.

This page describes the settlement protocol, not every route available through the [Relay platform](/how-relay-works). Each diagram follows one order and its solver; solvers are participants, not a single protocol-wide operator.
...
1. **User agrees to an order** — The user specifies what they want (e.g., bridge 1 ETH from Optimism to Base). An application obtains a solver's commitment; quoting and order distribution sit outside the settlement protocol.
...
4. **Oracle returns attestation** — The Oracle returns a signed EIP-712 attestation. The Oracle contract verifies an authorized signer; when a signer contract is used, that contract enforces its configured threshold. See the [Oracle trust model](/references/protocol/security#oracle).
