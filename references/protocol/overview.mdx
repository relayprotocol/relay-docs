...
Relay Protocol coordinates settlement for cross-chain transactions. Its settlement system, Relay Settlement, connects user deposits, fill verification, balance accounting, and withdrawals. A solver fills an order using its own capital; the protocol verifies the fill and accounts for settlement.

The [Relay platform](/what-is-relay) provides quotes, routing, and application tooling. This section explains the protocol's settlement mechanics and security model.

## Design Goals

Relay Settlement is optimized on five dimensions:
...
Relay Settlement handles settlement, not quoting or order distribution. It does not run an orderflow auction.

## Verification and Security

A solver fills an order, the [Oracle](/references/protocol/components/oracle) attests the deposit and fill, and the [Allocator](/references/protocol/components/allocator) authorizes withdrawals.

The protocol's [security model](/references/protocol/security) describes its deployed signers, withdrawal permissions, and [Security Council](/references/protocol/components/security-council). Review these verification and recovery mechanisms before integrating.
...
Relay Settlement records fills in real time on the Hub. A solver can batch authorized withdrawals to balance transaction costs against capital needs.
