...
_Defines the bridge function for crosschain asset transfers_
...
RelayBridge contract enabling crosschain bridging via Hyperlane
...
Hyperlane Mailbox contract for crosschain messaging
...
Calculates the fee required in native currency to dispatch a crosschain message

_Uses Hyperlane's quoteDispatch to estimate the crosschain messaging fee_
...
_Executes a crosschain bridge transaction with the following steps:
     ...
     4. Dispatches crosschain message via Hyperlane
