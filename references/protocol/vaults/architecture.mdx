...
standard. It is deployed on specific EVM network, for a specific asset,
represented by an ERC20. It is possible to use wrapped tokens (such as WETH) to
handle native assets (Relay provides a
...
yield pool and sent to the destination address. At the same time, the vault initiates the
...
In order to abstract the underlying bridges, Relay relies on the concept of
