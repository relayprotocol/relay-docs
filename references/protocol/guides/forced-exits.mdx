...
A forced exit allows a user to reclaim their deposited funds when a solver fails to fill an order **and** fails to refund. Unlike a [self refund](/references/protocol/refunds#self-refund) — which requires an Oracle attestation — a forced exit bypasses the Oracle entirely and interacts directly with onchain contracts.
