...
Once a deposit is attested (or if the depositor already has credit on the hub from a prior recovery), you can enumerate what's available to withdraw for a given recovery owner:

```bash
curl "https://api.relay.link/withdrawals/balances?recoveryAddress=0xDEPOSITOR_ADDRESS&ownerChainId=8453&limit=20"
```

- **`recoveryAddress`** — the depositor / owner address
