...
When an incoming request sets `requestPeerSignatures=true` and `PEERS` is configured, the oracle starts its local attestation and every configured peer request concurrently. The first response group whose canonical unsigned payload accumulates the configured number of unique allowlisted signatures is returned. The local oracle is not privileged, so a quorum can be satisfied entirely by remote peers.

Responses are combined only when their complete unsigned payloads and signing metadata match. The resolver rejects aggregation across differing attestation payloads, differing signing domains, differing array ordering, duplicate signers, non-allowlisted signers, multi-message responses where a signer is missing from any signature array, or responses that fail the endpoint schema.
...
```sh
ORACLE_SIGNERS=0xOracleOne;0xOracleTwo;0xOracleThree
ORACLE_SIGNERS_THRESHOLD=2
```

The service fails to start when:

- `ORACLE_SIGNERS_THRESHOLD` is not an integer, is less than `1`, or exceeds the number of unique entries in `ORACLE_SIGNERS`.
- Any entry in `ORACLE_SIGNERS` is not a valid nonzero address.
- The local signer address is not included in `ORACLE_SIGNERS` (when `ORACLE_SIGNERS` is configured).

<Warning>`ORACLE_SIGNERS_THRESHOLD` is a total signature count. A 2-of-3 oracle configuration uses `ORACLE_SIGNERS_THRESHOLD=2`, not `1`. Existing deployments that previously treated the threshold as an additional-peer count must increment the value together with the application rollout.</Warning>

When `ORACLE_SIGNERS` is omitted, the service preserves compatibility by waiting for the local operation and every configured peer instead of completing early at the threshold, and only peer responses that match the local unsigned result are aggregated.

Peer calls remain best-effort. If a peer times out, errors, fails response validation, or does not agree with a signable quorum, it is skipped and a structured warning is emitted. If no group reaches the threshold, the strongest valid partial signed group is returned after all sources settle, and a structured warning records the matched and required signature counts.
...
- `ORACLE_SIGNERS` is the semicolon-separated allowlist used to count unique oracle signatures during quorum aggregation. See [Peer oracles and quorum](/references/protocol/guides/third-party-oracle#peer-oracles-and-quorum) for behavior details.
- `ORACLE_SIGNERS_THRESHOLD` is the total number of unique allowlisted signatures required for a quorum. It defaults to `1` and is validated at startup against the size of `ORACLE_SIGNERS`.
