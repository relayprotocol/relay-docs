...
A single oracle can answer requests on its own, and the Relay Oracle service also supports peering across multiple oracle instances to collect signatures.
...
`ORACLE_SIGNERS_THRESHOLD` counts the local oracle's signature along with peer signatures. A threshold of `1` can be met by the local oracle alone, so a 2-of-3 configuration requires `2`.
