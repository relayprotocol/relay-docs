...
When an incoming request sets `requestPeerSignatures=true` and `PEERS` is configured, the oracle starts its local attestation and every configured peer request concurrently. The first response group whose unsigned payload accumulates the configured number of unique allowlisted signatures is returned. The local oracle is not privileged, so a quorum can be satisfied entirely by remote peers.
...
- `ORACLE_SIGNERS_THRESHOLD` is the total number of unique allowlisted signatures required for a valid quorum. It defaults to `1` and must not exceed the size of `ORACLE_SIGNERS`.
...
When `ORACLE_SIGNERS` is omitted, none of those three checks run and the threshold is ignored for quorum purposes. The service logs a startup warning in that case, so a threshold set without an allowlist enforces nothing.

<Warning>
**2026-08-06**: `ORACLE_SIGNERS_THRESHOLD` is now a total signature count, so a 2-of-3 configuration uses `2`, not `1`. It previously counted additional peer signatures and defaulted to `0`, and `0` is now rejected at startup. Deployments that set the value explicitly must raise it together with the application rollout.
</Warning>

Without the allowlist, the service preserves compatibility by waiting for the local operation and every configured peer instead of completing early at the threshold. Only peer responses that match the local unsigned payload are aggregated.
...
If no group reaches the threshold, the strongest valid partial group is returned once all sources settle, and a structured warning records the matched and required signature counts.
...
Aggregated signatures are consumed onchain by a single authorized signer address, which can be an EIP-1271 contract such as `RelayOracleMultisig`. See [Oracle Contract](/references/protocol/components/oracle#oracle-contract) for how thresholding works on that side.
