...
A single oracle can answer requests on its own, and the code also supports peering across multiple oracle instances to collect signatures.

When an incoming request sets `requestPeerSignatures=true` and `PEERS` are configured, the oracle starts its local attestation and every configured peer request concurrently.
...
Responses are combined only when their unsigned payloads and signing metadata match.

The oracle rejects aggregation across:
...
- `ORACLE_SIGNERS_THRESHOLD` is the total number of unique allowlisted signatures required for a valid quorum (defaults to `1`).
...
The code fails at startup when `ORACLE_SIGNERS_THRESHOLD` is set to something that is not an integer, or to an integer below `1`.

When `ORACLE_SIGNERS` is configured, the code also fails at startup when:
...
When `ORACLE_SIGNERS` is omitted, the threshold is ignored for quorum purposes, so a threshold set without an allowlist doesn't enforce anything. The code provides a startup warning when a threshold is set without the allowlist.
...
With an allowlist, if no group reaches the threshold, the strongest valid partial group is returned once all sources settle, and a warning records the matched vs required signature counts.
