...
| -------------------------- | ----------------------------------------------------------------------------------------------- | -------- |
| **vmType**                    | A string representing a supported vmType (`evm`  `svm`  `bvm`  `suivm` `tvm`)                                                   | ✅       |
...
You can either make your own adapter, as long as it adheres to the interface above or you can use one of the officials adapters developed and maintained by the Relay team. All SDK methods handle a viem wallet or adapted wallet. The viem wallet adapter is used by default when a [viem wallet](https://viem.sh/docs/clients/wallet.html) is passed in for convenience. Refer below for implementation details:
...
//In this example a keypair is loaded as the wallet, but if you have a connector like dynamic you can just fetch the connection from that library
...
// Create a TronWeb instance, using a provider like dynamiclabs is recommended if using this on the frontend
