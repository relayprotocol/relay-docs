...
| **user**       | The address depositing funds on the origin chain. Must be a valid address for the origin chain's virtual machine (e.g. a Solana address for a Solana origin, an EVM address for an EVM origin). Required unless `includeDefaultParameters` is true. | ✅ (❌ if `includeDefaultParameters` is true) |
| **recipient**  | The address receiving funds on the destination chain. Must be a valid address for the destination chain's virtual machine. Required unless `includeDefaultParameters` is true. | ✅ (❌ if `includeDefaultParameters` is true) |
...
| **options**    | Additional options that map directly to the [quote API](/references/api/get-quote-v2).                                                                                                                             | ❌                                            |
...
**Note** - Quotes are revalidated when being filled, clients should regularly fetch fresh quotes so that users are always submitting up to date quotes. See [Step Execution](/references/api/api_core_concepts/step-execution) for more information on executing quotes through the platform.
