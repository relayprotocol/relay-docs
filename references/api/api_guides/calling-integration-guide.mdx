...
<Steps>
  <Step title="Get a Quote">
    ...
    <Info>
      For `EXACT_INPUT` calls, integrators usually need their own proxy contract on the destination chain. Because Relay delivers a variable output amount, the proxy contract should read the received balance at execution time and construct any follow-up calldata from that balance before calling the final target contract.
    </Info>
    ...
    **Solution**: Verify the contract address in the approval matches the contract you're calling
    ...
    | `tradeType`           | string      | Use `EXACT_INPUT` or `EXACT_OUTPUT`, depending on whether you want to quote by input amount or required output amount.             |
...
<input className="relay-checkbox" type="checkbox" name="trade-type" value="yes"/> **Check tradeType** - Use `"EXACT_OUTPUT"` when the destination call needs a precise amount, or `"EXACT_INPUT"` when spend should be fixed and output can vary

<input className="relay-checkbox" type="checkbox" name="exact-input-proxy" value="yes"/> **Plan for `EXACT_INPUT` execution** - For `"EXACT_INPUT"` calls, use a destination proxy contract that reads the variable output balance and builds any follow-up calldata at runtime
...
- [**Gasless Execution**](/features/gasless-execution): Execute gasless transactions where the transaction signer and gas payer are different addresses.
