...
<input className="relay-checkbox" type="checkbox" name="trade-type" value="yes"/> **Check tradeType** - Use `"EXACT_OUTPUT"` for a precise output, or `"EXACT_INPUT"` to cap spend

<input className="relay-checkbox" type="checkbox" name="exact-input-proxy" value="yes"/> **Plan for `EXACT_INPUT` execution** - Use a destination proxy contract that reads the delivered balance and builds follow-up calldata at runtime
...
- [**Platform Fees**](/references/api/api_core_concepts/fees): Understand the fees associated with using the Relay API.