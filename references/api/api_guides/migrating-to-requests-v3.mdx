...
This migration changes the platform API response and query surface. The `v3` endpoint suffix does not mean a request uses a particular version of [Relay Protocol](/references/protocol/overview).
...
No `Retry-After` is emitted on `429` from `GET /requests/v2`. For guidance on reducing your request volume, see [Handling Rate Limits](/references/api/api_core_concepts/handling-rate-limits).
...
<AccordionGroup>
  ...
  <Accordion title="Filtering, search & sorting" icon="magnifying-glass">
    ...
    | **Status / routing** | `status`, `failReason`, `refundFailReason`, `apiKey`, `requestType`, `referrer` |
    ...
    | **Other** | `features`, `includeChildRequests`, `filters` (AND/OR), `sortBy`, `sortDirection`, `includeAuthenticatedData`, `includeTotal` |
    ...
    **Advanced filtering (AND / OR / negation / null-checks).** The `filters` parameter takes a JSON-encoded array of groups. Fields **within** a group are AND'd; the groups themselves are OR'd. Any key can be negated with a `not:` prefix, a field's value may be an array (OR within that field), and a value of `null` is a null-check ("is empty" / "is not empty") instead of an equality test. Top-level parameters still apply.
    ...
    # null-checks: requests with no deposit address, or with any deposit address
    GET /requests/v3?filters=[{"depositAddress":null}]
    GET /requests/v3?filters=[{"not:depositAddress":null}]
    ```

    <Note>OR groups support identity, address, chain, currency, and status-style fields. Range filters and `features` are top-level only. Null-checks work on any recognised `filters` key, including the composite `user` field and the case-insensitive `currencyInSymbol` / `currencyOutSymbol` fields. `null` cannot be combined with real values in an array for the same key — that returns `400`.</Note>

    **Referrer filtering (owner-scoped).** The `referrer` filter — as a top-level query parameter or as a `referrer` / `not:referrer` key inside any `filters` group — matches the exact value supplied at quote time. Because referrer values are supplied by the caller and are guessable, a referrer filter **must be paired with the top-level `apiKey` parameter**, and every API key you list must belong to the authenticating integrator. Empty `referrer` values are rejected. Requests without the `apiKey` pairing return `400`; requests whose supplied keys are not owned by the caller integrator (or which cannot resolve to an integrator identity) return `403`.

    ```bash
    # exact referrer match; apiKey pairing scopes the result to your own requests
    GET /requests/v3?referrer=my-app|user-123&apiKey=YOUR_KEY

    # same requirement when the key appears inside filters (including not:)
    GET /requests/v3?filters=[{"referrer":"my-app|user-123"}]&apiKey=YOUR_KEY
    GET /requests/v3?filters=[{"not:referrer":"other-app"}]&apiKey=YOUR_KEY,YOUR_OTHER_KEY
    ```

    **Removed & changed parameters.**
    - `hash` is removed; use `term`, or the targeted `depositTxHash` / `fillTxHash` / `refundTxHash`.

    **Total match count.** Pass `includeTotal=true` to receive the exact total count of matching requests as `total` on the response (independent of `limit` and `continuation`). Omit it — or set `includeTotal=false` — when you only need the current page; computing the total adds meaningful query latency, so it is opt-in.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Moved, added & nulled fields" icon="arrows-up-down-left-right">
    ...
    - `referrer` moves into **`data.referrer`** and is returned **only when `includeAuthenticatedData=true`**. It remains a supported query filter (see [Filtering, search & sorting](#what-changed)) but referrer-filtered queries must pair with the `apiKey` parameter using keys owned by the authenticating integrator.
    ...
    - **`supersededByRequestId`** is **promoted to the root**; it previously lived at `data.supersededByRequestId` in v2. When a deposit-address request is regenerated (currently Bitcoin/BVM only), the original request's `supersededByRequestId` points to the most recent regenerated `requestId` that owns the sweep and fill. `null` otherwise, including on regenerations from every other chain — track those by `depositAddress` (see [Handling Quote Regeneration](/features/deposit-addresses#handling-quote-regeneration)).
    ...
    - **`data.apiKeyName`** is **added** — the display name of the API key that created the request. Returned **only when `includeAuthenticatedData=true`** and the caller authenticates as the request's owning integrator (same gate as `data.referrer` and `data.platformFee`); `null` when the API key has no name set.
    - **`data.blockedAddresses`** is **added** — an array of the wallet address(es) that failed screening and caused a `BLOCKED_WALLET` failure. Returned **only when `includeAuthenticatedData=true`** and the caller authenticates as the request's owning integrator (same gate as `data.referrer`, `data.platformFee`, and `data.apiKeyName`); `null` on non-`BLOCKED_WALLET` failures and when no addresses were recorded. Screening-provider scores and reasons are never returned.
...
<AccordionGroup>
  <Accordion title="Renamed & restructured" icon="pen-to-square">
    ...
    | `data.supersededByRequestId` | `supersededByRequestId` (root) |
  </Accordion>
  <Accordion title="Removed" icon="trash">
    ...
    | `referrer` (query parameter, cross-integrator) | supported as an owner-scoped filter; must pair with the `apiKey` parameter using keys owned by the authenticating integrator. Still returned in the response body only when `includeAuthenticatedData=true` |
  ...
  <Accordion title="Added" icon="plus">
    ...
    | `data.apiKeyName` | display name of the API key that created the request; returned only with `includeAuthenticatedData=true` for the owning integrator |
    | `data.blockedAddresses` | address(es) that failed screening on `BLOCKED_WALLET` failures; returned only with `includeAuthenticatedData=true` for the owning integrator |
