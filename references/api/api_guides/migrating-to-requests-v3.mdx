...
<AccordionGroup>
  ...
  <Accordion title="App fees — consolidated into one object" icon="layer-group">
    ...
    <Note>
      App fees are authenticated-only on v3. They are returned only when the caller passes `includeAuthenticatedData=true` and authenticates with an API key belonging to the request's owning integrator — the same gate as `data.referrer`, `data.platformFee`, and `data.apiKeyName`. For every other caller, `quoted` and `actual` keep their arity but `recipient`, `bps`, `amount`, `amountFormatted`, and `amountUsd` are `null`; the corresponding `data.fees.{quoted,actual}.app` and `data.feeSponsorship.{quoted,actual}.components.app` (along with the `sponsored`, `sponsoredTotal`, and `userPaysTotal` aggregates that contain them) are `null` on the same terms. `null` reads as "withheld" — a `0` would assert a fee amount that is not true. Requests created without an API key are exempt from this rule.
    </Note>
  </Accordion>
  ...
  <Accordion title="Moved, added & nulled fields" icon="arrows-up-down-left-right">
    ...
    - **`data.appFees`** is now **authenticated-only** in v3. When the caller passes `includeAuthenticatedData=true` and authenticates as the request's owning integrator, `data.appFees.{quoted,actual}[]` return real values; otherwise their `recipient`, `bps`, `amount`, `amountFormatted`, and `amountUsd` fields are `null` and entries keep their arity so the response shape is preserved. The `app` component of `data.fees.{quoted,actual}` and `data.feeSponsorship.{quoted,actual}.components.app` (plus the `sponsored`, `sponsoredTotal`, and `userPaysTotal` totals that contain them) are `null` on the same terms. Requests created without an API key are exempt.
