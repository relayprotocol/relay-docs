---
title: "ERC-4337"
description: "How to use ERC-4337 to execute crosschain transactions"
---

One can use ERC-4337 user operations to execute crosschain transactions. There are three possible interactions that one can have:
...
Interacting with the solver using ERC-4337 user operations requires two additional steps on top of the standard `/quote/v2` flow.

- The first requires sending an extra field in the `/quote/v2` request body, `userOperationGasOverhead`. This field indicates how much additional gas overhead will be necessary to include the user operation on the chain as compared to a normal EOA transaction.
...
In the following example, a Safe Smart Account is used with a Pimlico Paymaster, leveraging Relay to get the quotes and submit the user operation.
...
To execute user operations on the destinations, you can pass in the transaction details in the [txs](https://docs.relay.link/references/api/get-quote-v2#body-txs) array while calling the quote API.
...
- Pass the userOperationGasOverhead while making the `/quote/v2` call and also pass the destination chain Entry Point transaction encoded in the `txs` array.
