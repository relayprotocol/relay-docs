...
Webhooks are self-serve in the [Relay Dashboard](https://dashboard.relay.link). Each API key can have a single webhook endpoint, and any requests tied to that key will post events to it. To configure one:

1. Sign in to the [Relay Dashboard](https://dashboard.relay.link).
2. Open the **API keys** tab.
3. Open the actions menu on the key you want to configure and select **Configure Webhook**.
4. Enable the webhook and enter your **Webhook URL** — an HTTPS endpoint to POST to when the status of a request changes (e.g. `https://my.server/receive-relay-event`).
5. Optionally add **Custom Headers** to include on every webhook request (e.g. `Authorization: Bearer 0x123`), then save.
...
The `details`, `failReason`, and `refundFailReason` fields mirror the same fields returned by [`GET /intents/status/v3`](/references/api/get-intents-status-v3):

| **Field** | **Description** |
| --- | --- |
| `details` | Additional status context (e.g. decoded revert data on failures). `null` when no extra detail is available. |
| `failReason` | Reason a fill failed. Returns `"N/A"` on non-failure statuses. On `failure`, `fallback`, and `refund` statuses, returns the recorded reason or `"UNKNOWN"` when no specific reason is available. See [`/intents/status/v3`](/references/api/get-intents-status-v3) for the enum. |
| `refundFailReason` | Reason a refund leg failed. Defaults to `"N/A"` when no refund-leg failure is recorded. See [`/intents/status/v3`](/references/api/get-intents-status-v3) for the enum. |

For deposit-address requests, events are sent under the `requestId` that owns the fill. If Relay regenerates the quote, events carry the new `requestId` and nothing is sent for the quote-time one. Match these events to your order with `depositAddress.address`, and to the funding transfer with `depositAddress.depositTxHash`. See [Tracking Transactions](/features/deposit-addresses#tracking-transactions).
