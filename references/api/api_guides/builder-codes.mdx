...
See [Wallet Detection](/references/api/api_core_concepts/wallet-detection) for how **`explicitDeposit`** works, and the [Get Quote reference](/references/api/get-quote-v2#body-explicit-deposit) for the parameter. Builder codes only work for quotes that route through the Relay Depository contract; same-chain swaps don't use the deposit flow and can't carry a builder code.
