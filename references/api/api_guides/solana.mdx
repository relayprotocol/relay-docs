...
Relay now fully supports depositing & withdrawing to Solana from any supported EVM chain. This is a great way to get users funds on Solana to complete
...
|                      | `toCurrency` | Contract Address of Solana Token | Must be a valid Solana token address. Do **not** use an EVM address. Relay supports all tokens tradeable on [Jupiter](https://jup.ag/). |
| Withdraw from Solana | `chainId`    | `792703809`                      | Chain ID assigned to access Solana for Relay's tools.                                                                               |
|                      | `currency`   | Contract Address of Solana Token | Must be a valid Solana token address. Do **not** use an EVM address. Relay supports all tokens tradeable on [Jupiter](https://jup.ag/). |
...
The table below lists the most frequently used Solana tokens & their
contract addresses. Relay supports all tradeable tokens on
...
|                      | `destinationCurrency` | Contract Address of Solana Token | Must be a valid Solana token address. Do **not** use an EVM address. Relay supports all tokens tradeable on [Jupiter](https://jup.ag/). |
...
|                      | `originCurrency`      | Contract Address of Solana Token | Must be a valid Solana token address. Do **not** use an EVM address. Relay supports all tokens tradeable on [Jupiter](https://jup.ag/). |

### Execution

Once you have the necessary information, you can start utilizing the Relay API for
full Solana withdrawal and deposit support. To get started, check out the
[execution steps](/references/api/api_core_concepts/step-execution) of the API. Then when you're
