...
## 2026-07-02 — Opt-in solver signature on quote responses

**Added** — `POST /quote` and `POST /quote/v2`: a new optional `includeProtocolSignature` boolean. When set to `true`, the response includes `protocol.v2.orderSignature` — the solver's ECDSA signature over the 32-byte `protocol.v2.orderId`. Recover the signer with `ecrecover` to verify onchain that an `orderId` passed to `depositErc20` corresponds to a real solver commitment. The flag is off by default; enabling it adds a short signing round trip to quote generation, and the quote fails with `"Failed to generate protocol order"` if the signer does not respond in time.
