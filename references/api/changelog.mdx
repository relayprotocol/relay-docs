...
## 2026-08-18 — App fees are authenticated-only on `GET /requests/v3`; private-integrator redaction on `GET /requests` and `GET /requests/v2`

**Behavior change** — `GET /requests/v3`: app fee data is now returned only when the caller passes `includeAuthenticatedData=true` and authenticates with an API key belonging to the request's owning integrator — the same gate that already covers `data.referrer`, `data.platformFee`, and `data.apiKeyName`. For every other caller, `data.appFees.quoted[]` and `data.appFees.actual[]` still return one entry per app fee configured on the request, but `recipient`, `bps`, `amount`, `amountFormatted`, and `amountUsd` are `null`. The `app` component of `data.fees.{quoted,actual}` and `data.feeSponsorship.{quoted,actual}.components.app` are `null` on the same terms, along with the `sponsored`, `sponsoredTotal`, and `userPaysTotal` aggregates that would otherwise let the app fee be recovered by subtraction. Requests created without an API key are exempt from this rule — with no owning integrator to authenticate as, their app fees remain public. Response shape is unchanged. See the [migration guide](/references/api/api_guides/migrating-to-requests-v3#what-changed).

**Behavior change** — `GET /requests` and `GET /requests/v2`: for requests owned by integrators whose app fees are kept private, `appFees[]`, `paidAppFees[]`, the `app` component of `expandedPriceImpact.{quoted,actual}` (plus the sibling `sponsored` total), and `feeSponsorship.{quoted,actual}.components.app` (plus the `sponsoredTotal` and `userPaysTotal` phase totals) are returned with `null` values in place of the recipient, bps, amount, and USD fields. Unlike v3, this does **not** require `includeAuthenticatedData=true` — authenticating with one of the owning integrator's own API keys is enough to see the real values. Requests owned by other integrators are unaffected. Response shape is preserved.
