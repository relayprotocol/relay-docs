...
**Behavior change** — `POST /quote/v2`: quote attempts that fail because an upstream RPC provider returns a transient infrastructure error now return `errorCode: "REQUEST_TIMED_OUT"` when the upstream message indicates a timeout, or `errorCode: "RPC_HTTP_ERROR"` for other transient RPC failures. Previously these were misclassified as `errorCode: "DESTINATION_TX_FAILED"`, which incorrectly suggested an on-chain revert and was treated as non-retryable. `DESTINATION_TX_FAILED` continues to be returned for genuine destination reverts where the RPC provides real revert data. Both new codes are retryable — see [Handling Quote Errors](/references/api/api_core_concepts/handling-errors).
