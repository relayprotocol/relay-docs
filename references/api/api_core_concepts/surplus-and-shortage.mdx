...
An origin operation is the source-chain part of a bridge or swap. Its output determines the amount available for the cross-chain order; a solver uses its own capital to fill on the destination chain. There are three possible outcomes:
...
2. **Surplus**: The origin operation produces more than the quoted input amount.
3. **Shortage**: The origin operation produces less than the quoted input amount.
...
2. **Surplus**: The deposited amount is more than expected. The quoted rate is used as a heuristic to determine whether to proceed with the operation.
...
3. **Shortage**: The deposited amount is less than expected. The quoted rate is used as a heuristic to determine whether to proceed with the operation.
Specifically, if the solver can guarantee that the input amount can be swapped for at least the quoted rate of the expected amount, the operation continues. 
