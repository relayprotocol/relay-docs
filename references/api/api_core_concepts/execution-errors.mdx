...
| `TRANSACTION_NOT_INCLUDED` | The fill transaction was broadcast successfully and a transaction hash was recorded, but the transaction was not included on-chain before Relay's retry and gas-bump window elapsed. The transaction may still confirm afterwards, so the recorded hash is worth checking. |
