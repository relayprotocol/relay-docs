...
| ---------- | ------------ |
| `DEPOSIT_ADDRESS_MISMATCH` | The deposit was detected at an address that doesn't match the one issued for this request. |
...
| `TOO_LITTLE_RECEIVED` | The on-chain swap executed, but the output amount was below the guaranteed minimum. |
...
### On-Chain Execution & Revert Errors
...
| `ORDER_IS_CANCELLED` | The order was cancelled on-chain before it could be filled. |
