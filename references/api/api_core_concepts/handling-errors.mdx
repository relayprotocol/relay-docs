...
Expected errors return **HTTP 400** (Bad Request) and indicate issues with the request itself. Unexpected errors return **HTTP 500** (Internal Server Error) and indicate infrastructure or downstream failures.

## Expected Errors

These are known validation and routing issues that developers should gracefully handle in their integrations. These errors return **HTTP 400**:

| Error Code                                    | Description                                                                |
| --------------------------------------------- | -------------------------------------------------------------------------- |
| `AMOUNT_TOO_LOW`                              | The swap output amount is too small to cover fees required to execute the swap. This can occur when chain-specific fees (e.g. Hyperliquid's deposit fee) exceed the output amount. |
...
These indicate infrastructure or downstream failures and are not common. These errors return **HTTP 500**:
