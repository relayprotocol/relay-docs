...
<Tip>`REQUEST_TIMED_OUT` and `RPC_HTTP_ERROR` indicate transient upstream infrastructure issues, not problems with your request. Retry with backoff before surfacing a failure to the user.</Tip>
...
| Error Code                       | Description                                                               |
| -------------------------------- | ------------------------------------------------------------------------- |
| `DESTINATION_TX_FAILED`          | The relay transaction reverted on the destination chain with real revert data returned by the destination RPC. Transient upstream RPC failures are returned as `REQUEST_TIMED_OUT` or `RPC_HTTP_ERROR` instead. |
