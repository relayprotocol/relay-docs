...
The first supported type is `EXACT_INPUT`. In this order type, the user specifies how much of the input currency they want to provide for a swap or bridge. 
...
`EXPECTED_OUTPUT` is recommended for when an exact output amount is not necessary, or if calls do not need to be executed on destination.
...
The user is told how much of the origin token this will require to receive exactly the output amount. If the exact requested amount out cannot be filled, the request fails and the transaction reverts on origin. 
This trade type automatically accounts for any associated fees including relay fees, app fees, and gas fees.
`EXACT_OUTPUT` is recommended for bridge requests that involve destination calls, like minting using an exact erc20 amount
