...
Relay supports a variety of step IDs that are chained together based on the optimal route for execution. Each step has a unique `id` that identifies the type of action required. Understanding these step IDs helps you build better UX and handle each step appropriately.

### Transaction Steps

| Step ID | Description | When Used |
|---------|-------------|-----------|
| `deposit` | Deposit funds to the relayer to execute the swap | When the Relay solver holds the destination token (major tokens). Single transaction flow. |
| `approve` | Approve the transfer of an ERC20 token | Required before `deposit` or `swap` for tokens the solver doesn't already hold. |
...
| Step ID | Description | When Used |
|---------|-------------|-----------|
| `authorize` | Authorize the solver to spend funds from your wallet | Protocol-level permits. |
| `authorize1` | Sign to approve swap of tokens | Cross-chain permits using Permit2 or TransferWithAuthorization (EIP-3009). Main flow for cross-chain permit operations. |
| `authorize2` | Sign to approve transfer of tokens | Same-chain swap permits using PermitBatchWitnessTransferFrom. |
...
| Flow | Description |
|------|-------------|
| `deposit` | Single step for major tokens the solver holds |
| `approve` → `deposit` | Approval needed before deposit for tokens solver doesn't hold |
| `approve` → `swap` | Same-chain swap requiring approval |
| `authorize1` | Cross-chain permit-based transfer (gasless) |
| `authorize1` → `authorize2` | Cross-chain swap with both authorization steps |
...
An approval transaction to approve the transfer of an ERC20 token. Always followed by an additional step (`deposit` or `swap`) for tokens that the solver doesn't hold.
...
Sign authorization for cross-chain operations. Uses Permit2 or TransferWithAuthorization (EIP-3009) for gasless token transfers.

<Accordion title="TransferWithAuthorization Example (EIP-3009)">
```json
{
  "id": "authorize1",
...
<Accordion title="Permit2 Example">
```json
{
  "id": "authorize1",
  "action": "Sign authorization",
  "description": "Sign to approve swap of PENGU for ETH",
  "kind": "signature",
  "items": [
    {
      "status": "incomplete",
      "data": {
        "sign": {
          ...
          "value": {
            "permitted": [
              {
                "token": "0x01e6bd233f7021e4f5698a3ae44242b76a246c0a",
                "amount": "10000000000000000000000"
              }
            ],
            "nonce": "44088590595258",
            "deadline": 1764948343,
            "spender": "0xbbbfd134e9b44bfb5123898ba36b01de7ab93d98",
            "witness": {
              "relayer": "0xf70da97812cb96acdf810712aa562db8dfa3dbef",
              "refundTo": "0xf70da97812cb96acdf810712aa562db8dfa3dbef",
              "nftRecipient": "0xf70da97812cb96acdf810712aa562db8dfa3dbef",
              "call3Values": [
                {
                  "target": "0x01e6bd233f7021e4f5698a3ae44242b76a246c0a",
                  "allowFailure": false,
                  "value": "0",
                  "callData": "0x095ea7b3..."
                }
        ...
        "post": {
          ...
          "body": {
            "kind": "permit2",
            "requestId": "0xbfcb68c255e4c596d0b984454ea29f4bbc88f13f598a184aed0e624797c8f83b",
            "api": "swap"
          }
        }
      },
      "check": {
        "endpoint": "/intents/status?requestId=0xbfcb68c255e4c596d0b984454ea29f4bbc88f13f598a184aed0e624797c8f83b",
        "method": "GET"
      }
    }
  ]
