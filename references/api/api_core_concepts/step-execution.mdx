...
description: "How step execution works when driving Relay directly from the API"
---

<Tip>
  Tip: For advanced integrations, we recommend using Relay APIs. For simpler use
  cases, the [Relay SDK](/references/relay-kit/sdk/installation) automates steps and
  provides helpful callbacks, reducing manual effort.
</Tip>

## Step Execution using the API

The [Relay platform](/references/api/overview) returns the steps needed to execute a quote, such as submitting a deposit transaction or signing a message. Follow the transaction targets, calldata, and amounts returned by the [quote endpoint](/references/api/get-quote-v2); they depend on the route and deposit method.

Step descriptions are display text, not a specification of fund ownership or settlement. Some API responses retain legacy “relayer” wording; use **`id`**, **`kind`**, and **`items`** to drive execution, and see [Input Validation](/references/api/api_core_concepts/input-validation) for verifying protocol deposits.

The flow looks like this:

<Steps>
  <Step title="Fetching Steps" icon="server">
    Call the [Get Quote API](/references/api/get-quote-v2) to start your
    execution.
  </Step>
...
```javascript Signing EIP712 expandable
...
| ------------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------- |
| [`deposit`](#deposit)     | Submit the quoted deposit for cross-chain execution   | Used for cross-chain swaps and bridges.                                             |
| [`approve`](#approve)     | Approve the transfer of an ERC20 token                 | Used before `deposit` or `swap` when the quoted flow requires a token allowance. |
...
| [`send`](#send)           | Send funds to the recipient                            | When sending the same asset on the same chain to a different recipient.             |
...
| [`authorize2`](#authorize2)   | Sign to approve transfer of tokens         | Same-chain ERC-20 → native swap permits (Permit2). Same-chain ERC-20 → ERC-20 swaps return `approve` → `swap` transactions instead; see [Gasless Swaps](/features/gasless-swaps#permit-based-gasless). |
...
| Flow                                                | Description                                     |
| --------------------------------------------------- | ----------------------------------------------- |
| [`deposit`](#deposit)                               | Single step for cross-chain swaps/bridges       |
| `swap`                                              | Single step for same-chain swaps                |
| [`approve`](#approve) → [`deposit`](#deposit)       | Approval needed before deposit for ERC20 tokens |
| [`approve`](#approve) → `swap`                      | Same-chain swap requiring approval              |
| [`authorize1`](#authorize1)                         | Cross-chain permit-based transfer (gasless)     |
...
Submit the deposit transaction returned in the quote. A solver uses its own liquidity to complete the cross-chain fill; the deposit target depends on the quoted route. The following historical response illustrates the step shape, not an address to reuse for new deposits.
...
```json expandable App Fee Claim Example
{
  "id": "authorize",
...
Sign authorization for same-chain ERC-20 → native swap permits using PermitBatchWitnessTransferFrom. Same-chain ERC-20 → ERC-20 swaps do not use this step; they return `approve` and `swap` transaction steps even when **`usePermit: true`** is set.
