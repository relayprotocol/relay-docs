...
The Refund currency can vary by route. Generally, Relay will refund the user in the original currency they sent. Usually this is one of the currencies in the `solverCurrencies` in the [chain api response](/references/api/get-chains), which are usually native or stable coins.
