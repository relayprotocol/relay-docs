...
The EVM protocol deposit flow sends funds to Relay Depository contracts.
For ERC20 token deposits, there are two possible transaction flows:
...
1. User transfers tokens to the Depository with the order ID encoded in
...
```
✅ Step 1: Transfer USDC to the Depository with the order ID encoded in calldata
