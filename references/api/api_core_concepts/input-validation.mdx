...
If you need the check to happen inside a contract rather than in your backend, use `protocol.v2.orderSignature`. It is a solver's ECDSA signature over the 32 raw bytes of the order ID, following EIP-191. The signing address in the example below is `0xf70da97812cb96acdf810712aa562db8dfa3dbef`; the signer does not determine the quote's deposit destination.

Pin the expected signer in your integration using independently verified configuration. Do not accept an arbitrary signer just because it appears in the same quote you are validating.
...
require(ECDSA.recover(digest, orderSignature) == EXPECTED_SOLVER_SIGNER, "bad signature");
```
