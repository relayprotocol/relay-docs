...
When Relayers execute transactions on behalf of users, they do so via a multicall contract (to be specific, Relay uses Vectorized's gas-optimized [Multicaller](https://github.com/Vectorized/multicaller) contract). This protects the Relayer from malicious transactions.
...
For authenticated actions, you can do those by passing in proof that the user authenticated the action. Currently, you need to build custom signature authentication into your contract, however, in the near future Relay will be adding support for more standardized flows, such as:
...
Relay will soon add functionality to the SDK to handle this flow completely automatically, but until then, you can do it manually like this:
