...
### No API Key Required
Relay's API is free and open to use — no API key or registration is needed to get started. Simply start making requests to `https://api.relay.link`. API keys are optionally available for integrators who need [higher rate limits](/references/api/api-keys).
</Callout>
