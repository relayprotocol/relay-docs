---
title: 'Quickstart'
description: 'Execute your first crosschain transaction in under 5 minutes.'
---

import StatusWebsockets from "/snippets/StatusWebsockets.mdx";

Relay is multichain payments infrastructure. You define the **Intent** (what the user wants), and Relay coordinates the **Execution** (how to get there).
...
<Steps>
  <Step title="Configure">
    To see how fast and simple Relay makes transacting crosschain, let's bridge real assets between inexpensive L2s—Base and Arbitrum.
  ...
  <Step title="Quote">
    ...
    The [quote endpoint](/references/api/get-quote-v2) handles all of your use cases, whether it’s a [**bridge**](/use-cases/bridging), [**swap**](/use-cases/cross-chain-swaps), or [**crosschain call**](/use-cases/calling). It calculates fees, finds the best route, and generates the transaction data.
  ...
  <Step title="Monitor">
    ...
    ## Status Lifecycle
    The diagram below illustrates the journey of a crosschain transaction through Relay's infrastructure and the corresponding API statuses. Below are descriptions for each status. Refer to the [Get Status](/references/api/get-intents-status-v3) endpoint for a full list of statuses and what they mean.
    ...
    - `success`: The Relay Solver successfully executed the "Fill Tx", and the funds have reached the destination.
    
    ![Status Lifecycle](/images/status-lifecycle.png)

    The diagram also depicts paths where a transaction may end up in a failure or reversion state if the fill cannot be completed. Learn more about how Relay helps you handle [reversions](https://docs.relay.link/references/api/api_core_concepts/refunds).
  </Step>
  <Step title="Optimize">
    You have successfully executed your first crosschain transaction with Relay!
    Check out some of the advanced features Relay offers to customize the experience:
...
Experiment with the Quickstart code in the [interactive sandbox](https://codesandbox.io/p/sandbox/github/GiselleNessi/relay-demo). You can fork or clone it to build your own project with Relay.

### See Also
- [**Bridging & Onboarding**](/use-cases/bridging): Learn more about instant crosschain deposits and withdrawals.
