...
description: "Crosschain payments infrastructure to connect users and merchants. One seamless experience."
---

Relay is crosschain payments infrastructure that enables seamless payments between users and merchants. Users pay in any token on any chain. Merchants receive any token or stablecoin as settlement. One integration to facilitate it all — instant, transparent, and low-fee.
...
Users connect their wallet, review the transaction, and confirm. Relay handles crosschain routing in the background. Compatible with WalletConnect and all major wallet providers.
...
**[Fast Fill](/features/fast-fill)** — Accelerate settlement by having Relay solvers pre-fill requests before the origin transaction finalizes onchain. Users and merchants see near-instant completion.
...
Relay is decentralized infrastructure. Users initiate transactions. Users maintain control of their assets throughout the payment flow. Funds are held in onchain escrow — not by Relay. The protocol coordinates execution through smart contracts. No single entity controls the network. Relay never takes custody of user funds.

All transactions are screened in real time against professional-grade sanctions and risk databases, including Chainalysis. Addresses linked to malicious activity are automatically blocked to ensure regulatory compliance. Learn more about [Relay's compliance practices](/security/compliance).

## Trusted by Leading Teams

Relay powers crosschain transactions for MetaMask, OpenSea, Coinbase Commerce, and other leading platforms. The protocol operates with 99.9%+ transaction reliability across 85+ networks with sub-3-second settlement on most routes.
