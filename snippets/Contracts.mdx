...
if (!window[key].fetching) {
  ...
  if (window[key].data) {
    setTimeout(() => {
      console.log('APPENDING CONTRACTS', window[key].data)
...
<table style={{ width: "100%", borderSpacing: "48px" }} className="relay-table">
  ...
  <tbody id={id} style={{width: "100%"}}>
  </tbody>
  <div className="loader-container" style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "200px", width: "100%" }}>
