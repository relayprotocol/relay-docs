...
You can programmatically fund your app balance through the Base address below. This app-balance flow uses a direct transfer to a solver address, not a Relay Protocol Depository deposit. Appending specific calldata identifies the account to credit.

#### How It Works

The platform indexes the transfer and credits your app balance using the appended calldata. This method uses a fixed 12-character prefix and suffix (`012345abcdef`) with the middle portion specifying which address to credit:
...
#### App Balance Funding Address

Use this address only for the app-balance funding flow described here. For transaction deposits, use the steps returned by the quote API instead.
...
const APP_BALANCE_FUNDING_ADDRESS = '0xf70da97812cb96acdf810712aa562db8dfa3dbef' as const
...
const transferData = encodeFunctionData({
  ...
  args: [APP_BALANCE_FUNDING_ADDRESS, amountInWei]
})
