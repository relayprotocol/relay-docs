...
| Pausable + monitored      | Contract can be paused and active monitoring with sub-5-minute response is in place                                                          | ×0.75  |
...
An attacker uses a flash loan to reenter the withdrawal function and drain some of the protocol's funds in a single transaction. No special permissions required.
