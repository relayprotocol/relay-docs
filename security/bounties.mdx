...
Email [support@relay.link](<mailto:support@relay.link>). Send one email for each vulnerability. Do not combine several findings in one report.

Use a clear subject line that names the affected component and the issue. Example: `Vulnerability report: Approval Proxy signature replay`.

### What to include

Put all relevant information in the first email. We review the report as submitted. Include:

- A description of the vulnerability and the affected contract, service, or endpoint.
- Step-by-step reproduction instructions.
- A runnable PoC where the severity requires one. See [PoC requirements](#poc-requirements).
- Supporting material: transaction hashes, addresses, logs, or screenshots.

A report without the required material is not complete. Review starts when we receive the complete report.

### What happens next

- We confirm receipt of most reports within 24 hours.
- We send next steps within 7 days of receipt.
- If you do not hear from us before then, our team is still reviewing. Do not send a second report or open a new ticket for the same finding. Duplicate tickets slow the review.
- If you have new information about a report you already sent, reply in the original email thread.
- A different vulnerability needs a new email.
