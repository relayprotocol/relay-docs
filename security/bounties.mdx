...
<Info>
  The **permanently unrecoverable** modifier (×1.5) applies when damage from an
  exploit cannot be reversed once it happens. This is different from how quickly
  the damage occurs (which is captured by severity tiers). This modifier is used
  if the funds or system state cannot be restored after the exploit, such as
  when funds are stolen and removed before any response is possible (e.g.,
  laundered or bridged out), or when a freeze is permanent with no way to pause
  or upgrade the contract.
</Info>
